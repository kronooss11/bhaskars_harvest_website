...
$(function() {
    ...
    // Store product details on cart icon click (excluding nav cart)
    $('.icon-cart').not('.nav-link').on('click', function(e) {
        e.preventDefault();
        console.log('Cart icon clicked');
        
        let productCode, productSizes;
        
        {
            console.log('On shop page');
            // Get from closest parent text-center div that contains the product info
            const productContainer = $(this).closest('.imgHolder').siblings('.text-center');
            ...
            productCode = productContainer.attr('data-product-code');
            productSizes = productContainer.attr('data-product-sizes');
            
            console.log('Shop page values:', {
                productCode: productCode,
                productSizes: productSizes
            });
        }
        
        // Only store if we have both values
        if (productCode && productSizes) {
            // Get existing cart items or initialize empty array
            let cartItems = JSON.parse(localStorage.getItem('cartItems') || '[]');
            
            // Check if item already exists in cart
            const existingItemIndex = cartItems.findIndex(item => 
                item.productCode === productCode && item.size === productSizes
            );

            if (existingItemIndex !== -1) {
                // Update quantity of existing item
                cartItems[existingItemIndex].quantity += 1;
                cartItems[existingItemIndex].timestamp = new Date().toISOString();
            } else {
                // Add new item
                const cartItem = {
                    productCode: productCode,
            ...
            console.log('Stored in localStorage:', cartItems);
            const alertEl = $('<div class="success-alert">Item added to cart!</div>');
            $('body').append(alertEl);
            setTimeout(() => alertEl.addClass('show'), 10);
            setTimeout(() => {
    ...
    $('.btnShop').on('click', function(e) {
        ...
        // Get product details
        const productCode = $('.productTextHolder').attr('data-product-code');
        const productSizes = $('input[name="size"]:checked').attr('data-product-sizes');
        const quantity = parseInt($('input[type="number"]').val()) || 1;
        ...
        if (productCode && productSizes && quantity > 0) {
            // Get existing cart items or initialize empty array
            let cartItems = JSON.parse(localStorage.getItem('cartItems') || '[]');
    ...
    // Check if page is in products folder
    if (window.location.pathname.includes('/products/')) {
        // Update price when radio button changes
        $('input[name="size"]').on('change', function() {
            var price = $(this).data('price');
            $('.price').text('₹' + price);
        });
        
        // Set initial price based on checked radio button
        var initialPrice = $('input[name="size"]:checked').data('price');
    ...
    async function displayCartItems() {
        ...
        try {
            ...
            // Fetch products using Netlify function
            ...
            console.log('Products fetched from Netlify function:', productsData.length);
            ...
            cartItems.forEach(cartItem => {
                ...
                const product = productsData.find(p => {
                    console.log('Comparing:', {
                        'Product Code from Airtable': p.fields['data-product-code'],
                        'Product Code from Cart': cartItem.productCode
                    });
                    return p.fields['data-product-code'] === cartItem.productCode;
                });
                    ...
                    const itemHtml = `
                        <tr class="align-items-center">
                            <td class="d-flex align-items-center border-top-0 border-bottom px-0 py-6">
                                <div class="imgHolder">
                                    <img src="images/${product.fields['image-link']}" alt="${product.fields['Product Name']}" class="img-fluid">
                                </div>
                                <span class="title pl-2">
                                    <a href="javascript:void(0);">${product.fields['Product Name']}
                                        <p class="d-inline product-size">(${cartItem.size})</p>
                                    </a>
                                </span>
                            </td>
                            <td class="fwEbold border-top-0 border-bottom px-0 py-6">₹${price}</td>
                            <td class="border-top-0 border-bottom px-0 py-6">
                                <input type="number" class="quantity-spinner" min="1" value="${cartItem.quantity}" 
                                    data-product-code="${cartItem.productCode}" 
                                    data-product-size="${cartItem.size}">
                            </td>
                            <td class="fwEbold border-top-0 border-bottom px-0 py-6">
                                ₹${itemTotal.toFixed(2)} 
                                <a href="javascript:void(0);" class="fas fa-times float-right delete-item"
                                    data-product-code="${cartItem.productCode}" 
                                    data-product-size="${cartItem.size}"></a>
                            </td>
                        </tr>
                    `;
    ...
    if (window.location.pathname.includes('cart-page.html')) {
        ...
        $(document).on('change', '.quantity-spinner', async function() {
            ...
            try {
                // Get product price from the price cell
                ...
                const subtotal = cartItems.reduce((total, item) => {
                    const $itemRow = $(`tr:has([data-product-code="${item.productCode}"][data-product-size="${item.size}"])`);
                    const itemPrice = parseFloat($itemRow.find('td:nth-child(2)').text().replace('₹', ''));
        ...
        $(document).on('click', '.delete-item', function() {
            ...
            $row.fadeOut(300, function() {
                ...
                if (cartItems.length === 0) {
                    $('.cartTable tbody').html('<tr><td colspan="4" class="text-center">Your cart is empty</td></tr>');
                }
            });
